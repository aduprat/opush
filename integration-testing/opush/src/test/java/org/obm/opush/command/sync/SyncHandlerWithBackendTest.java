...
public class SyncHandlerWithBackendTest {
	...
	public void testInitialSyncThenRecreatesAccountOnCalendars() throws Exception {
		...
		AnalysedSyncCollection.Builder configuredSyncCollection = AnalysedSyncCollection.builder()
				...
				.dataType(CALENDAR);
		opClient.run(syncBuilder.request(
			ClientSyncRequest.builder().addCollection(configuredSyncCollection.syncKey(initialSyncKey).build()).build()).build());
		SyncResponse syncResponse = opClient.run(syncBuilder.request(
			ClientSyncRequest.builder().addCollection(configuredSyncCollection.syncKey(firstAllocatedSyncKey).build()).build()).build());
		
		opClient.run(syncBuilder.request(
			ClientSyncRequest.builder().addCollection(configuredSyncCollection.syncKey(initialSyncKey).build()).build()).build());
		SyncResponse firstSyncResponse = opClient.run(syncBuilder.request(
			ClientSyncRequest.builder().addCollection(configuredSyncCollection.syncKey(newFirstAllocatedSyncKey).build()).build()).build());
		SyncResponse secondSyncResponse = opClient.run(syncBuilder.request(
			ClientSyncRequest.builder().addCollection(configuredSyncCollection.syncKey(newSecondAllocatedSyncKey).build()).build()).build());
		SyncResponse thirdSyncResponse = opClient.run(syncBuilder.request(
			ClientSyncRequest.builder().addCollection(configuredSyncCollection.syncKey(newThirdAllocatedSyncKey).build()).build()).build());
		mocksControl.verify();
	...
	private void sendEmailsToImapServer(int mailCount) throws InterruptedException {
		for(int i = 0; i < mailCount; i++){
			GreenMailUtil.sendTextEmail(mailbox, mailbox, "subject"+i, "body", greenMail.getSmtp().getServerSetup());
		}
		greenMail.waitForIncomingEmail(mailCount);
	}
	...
	public void testFetchDeletedMail() throws Exception {
		...
		SyncResponse secondSyncResponse = opClient.run(syncBuilder
			.request(ClientSyncRequest.builder().addCollection(
				AnalysedSyncCollection.builder().collectionId(inboxCollectionId)
	...
	public void testNoContentDispositionPartIsSentAsAttachment() throws Exception {
		...
		Set<MSAttachement> attachments = syncThenGetOnlyEmailAttachments();
	...
	@Test
	public void testSyncEmailWithMultipleNameHeader() throws Exception {
		testUtils.appendToINBOX(greenMailUser, "eml/asteriskGrouping.eml");
		...
		assertThat(FluentIterable.from(attachments)
			...
			})).containsOnly("BA + QS _ Assurance Décès pour Investisseur.pdf", "Flyer502.png");
	}
	
	@Test
	public void testSyncEmailWithNameHeaderContainingCharset() throws Exception {
		testUtils.appendToINBOX(greenMailUser, "eml/asteriskCharset.eml");
		...
		assertThat(FluentIterable.from(attachments)
			...
			})).containsOnly("éàyeah!@&.png");
	}
	
	@Test
	public void testSyncEmailWithNameHeaderNotContainingCharset() throws Exception {
		testUtils.appendToINBOX(greenMailUser, "eml/asteriskNoCharset.eml");
	...
	private Set<MSAttachement> syncThenGetOnlyEmailAttachments() throws AuthFault, Exception {
	...
	public void testOnlyOneOpushObmSyncConnectionUsed() throws Exception {
		...
		SyncResponse syncResponse = testUtils.buildWBXMLOpushClient(user, opushServer.getHttpPort(), httpClient).run(syncBuilder
			.request(ClientSyncRequest.builder()
				.addCollection(AnalysedSyncCollection.builder().collectionId(calendarCollectionId).syncKey(firstAllocatedSyncKey).dataType(CALENDAR)
					.options(SyncCollectionOptions.builder()
						.filterType(FilterType.ONE_WEEK_BACK)
						.build())
					.build())
				.addCollection(AnalysedSyncCollection.builder().collectionId(contactCollectionId).syncKey(firstAllocatedSyncKey).dataType(CONTACTS)
					.options(SyncCollectionOptions.builder()
						.filterType(FilterType.ONE_WEEK_BACK)
						.build())
					.build())
			...
			.request(ClientSyncRequest.builder()
				.addCollection(AnalysedSyncCollection.builder().collectionId(calendarCollectionId).syncKey(firstAllocatedSyncKey).dataType(CALENDAR)
					.options(SyncCollectionOptions.builder()
						...
						.build())
					.build())
				.addCollection(AnalysedSyncCollection.builder().collectionId(inboxCollectionId).syncKey(firstAllocatedSyncKey).dataType(EMAIL)
					.options(SyncCollectionOptions.builder()
						.filterType(FilterType.ONE_WEEK_BACK)
						.build())
					.build())
	...
	public void confidentialEventSensitivityNotModifiedByDevices() throws Exception {
		...
		opClient.run(syncBuilder
			.request(ClientSyncRequest.builder()
				.addCollection(AnalysedSyncCollection.builder().collectionId(calendarCollectionId).dataType(CALENDAR)
		...
		SyncResponse updatedSyncResponse = opClient.run(syncBuilder
			.device(user.device)
			.request(ClientSyncRequest.builder()
	...
	@Test
	public void syncShouldGetAllChangesWhenWindowingDifferentKindOfChange() throws Exception {
		SyncKey initialSyncKey = SyncKey.INITIAL_SYNC_KEY;
		SyncKey firstAllocatedSyncKey = new SyncKey("d6b223c4-c7ea-499e-8f65-d94e3121efb8");
		SyncKey secondAllocatedSyncKey = new SyncKey("0e5e9ebc-5210-423f-a15d-5d360c031220");
		SyncKey thirdAllocatedSyncKey = new SyncKey("279cf50e-9f28-4c92-8ac6-7d3e7cab1056");
		SyncKey fourthAllocatedSyncKey = new SyncKey("fee34558-8068-45b0-b741-d21711998019");
		int firstAllocatedStateId = 3;
		int secondAllocatedStateId = 4;
		int thirdAllocatedStateId = 5;
		int fourthAllocatedStateId = 6;
		int deletedEmails = 2;
		int newEmails = 4;
		
		userAccessUtils.mockUsersAccess(Arrays.asList(user));
		syncKeyTestUtils.mockNextGeneratedSyncKey(firstAllocatedSyncKey, secondAllocatedSyncKey, 
				thirdAllocatedSyncKey, fourthAllocatedSyncKey);
		...
		ItemSyncState fourthAllocatedState = ItemSyncState.builder()
				.syncDate(date("2012-10-10T17:28:17"))
				.syncKey(fourthAllocatedSyncKey)
				.id(fourthAllocatedStateId)
				.build();
		
		expect(dateService.getEpochPlusOneSecondDate()).andReturn(initialDate).anyTimes();
		expect(dateService.getCurrentDate()).andReturn(firstAllocatedState.getSyncDate());
		...
		syncTestUtils.mockCollectionDaoPerformSync(user.device, firstAllocatedSyncKey, firstAllocatedState, secondAllocatedState, inboxCollectionId);
		syncTestUtils.mockCollectionDaoPerformSync(user.device, secondAllocatedSyncKey, secondAllocatedState, thirdAllocatedState, inboxCollectionId);
		syncTestUtils.mockCollectionDaoPerformSync(user.device, thirdAllocatedSyncKey, thirdAllocatedState, fourthAllocatedState, inboxCollectionId);
		...
		expect(itemTrackingDao.isServerIdSynced(secondAllocatedState, inboxCollectionId.serverId(3))).andReturn(false);
		expect(itemTrackingDao.isServerIdSynced(secondAllocatedState, inboxCollectionId.serverId(4))).andReturn(false);
		expect(itemTrackingDao.isServerIdSynced(secondAllocatedState, inboxCollectionId.serverId(5))).andReturn(false);
		expect(itemTrackingDao.isServerIdSynced(secondAllocatedState, inboxCollectionId.serverId(6))).andReturn(false);
		...
		greenMail.deleteEmailFromInbox(greenMailUser, 1l);
		greenMail.deleteEmailFromInbox(greenMailUser, 2l);
		sendEmailsToImapServer(4);
		SyncResponse firstWindowingResponse = opClient.syncEmail(decoder, secondAllocatedSyncKey, inboxCollectionId, FilterType.THREE_DAYS_BACK, newEmails);
		SyncResponse secondWindowingResponse = opClient.syncEmail(decoder, thirdAllocatedSyncKey, inboxCollectionId, FilterType.THREE_DAYS_BACK, deletedEmails);
		
		mocksControl.verify();

		assertThat(firstWindowingResponse.getStatus()).isEqualTo(SyncStatus.OK);
		SyncCollectionResponse response1 = syncTestUtils.getCollectionWithId(firstWindowingResponse, inboxCollectionId);
		syncTestUtils.assertEqualsWithoutApplicationData(response1.getItemChanges(), ImmutableList.of(
				...
				.build(),
				ItemChange.builder()
				.serverId(inboxCollectionId.serverId(5))
				.isNew(true)
				.build(),
				ItemChange.builder()
				.serverId(inboxCollectionId.serverId(6))
				.isNew(true)
				.build()));

		assertThat(secondWindowingResponse.getStatus()).isEqualTo(SyncStatus.OK);
		SyncCollectionResponse response2 = syncTestUtils.getCollectionWithId(secondWindowingResponse, inboxCollectionId);
		assertThat(response2.getItemDeletions()).containsOnly(
				ItemDeletion.builder().serverId(inboxCollectionId.serverId(1)).build(),
	...
	@Test
	public void addInSyncShouldHaveBodyWhenTextBodyPreferenceAndMultipartMixed() throws Exception {
		...
		assertThat(mail.getAttachments()).hasSize(2);
		MSEmailBody body = mail.getBody();
		assertThat(body.getEstimatedDataSize()).isEqualTo(10);
		assertThat(body.isTruncated()).isFalse();
		assertThat(body.getBodyType()).isEqualTo(MSEmailBodyType.PlainText);
		assertThat(CharStreams.toString(new InputStreamReader(body.getMimeData().get(), Charsets.UTF_8))).isEqualTo("text plain");
	}
	
	@Test
	public void fetchInSyncShouldTruncateWhenTextBodyPreferenceAndMultipartMixed() throws Exception {
		testUtils.appendToINBOX(greenMailUser, "eml/textAndAttachments.eml");
		...
		assertThat(body.getBodyType()).isEqualTo(MSEmailBodyType.PlainText);
		assertThat(CharStreams.toString(new InputStreamReader(body.getMimeData().get(), Charsets.UTF_8))).isEqualTo("text");
	}

	@Test
	public void addInSyncShouldHaveTransformedBodyWhenTextBodyPreferenceAndHtmlMultipartMixed() throws Exception {
		testUtils.appendToINBOX(greenMailUser, "eml/htmlAndAttachments.eml");
		...
		assertThat(mail.getAttachments()).hasSize(2);
		MSEmailBody body = mail.getBody();
		assertThat(body.getEstimatedDataSize()).isEqualTo(11);
		assertThat(body.isTruncated()).isFalse();
		assertThat(body.getBodyType()).isEqualTo(MSEmailBodyType.PlainText);
		assertThat(CharStreams.toString(new InputStreamReader(body.getMimeData().get(), Charsets.UTF_8))).isEqualTo("html");
	}

	@Test
	public void fetchInSyncShouldTransformeBodyWhenTextBodyPreferenceAndHtmlMultipartMixed() throws Exception {
		testUtils.appendToINBOX(greenMailUser, "eml/htmlAndAttachments.eml");
	...
	@Test
	public void addInSyncShouldHaveMimeBodyWhenHtmlThenMimeBodyPreferenceButAttachmentsOnly() throws Exception {
		testUtils.appendToINBOX(greenMailUser, "eml/only_attachments.eml");
		...
		SyncResponse response = opClient.run(syncBuilder
			.request(ClientSyncRequest.builder()
				.addCollection(AnalysedSyncCollection.builder()
					...
					.options(SyncCollectionOptions.builder()
						.filterType(FilterType.ONE_WEEK_BACK)
						.bodyPreferences(ImmutableList.of(
	...
	@Test
	public void addInSyncShouldHaveMimeBodyWhenHtmlThenMimeBodyPreferenceAndMultipartMixed() throws Exception {
		testUtils.appendToINBOX(greenMailUser, "eml/textAndAttachments.eml");
		...
		assertThat(CharStreams.toString(new InputStreamReader(body.getMimeData().get(), Charsets.UTF_8)))
			.contains("<html><body>text plain</body></html>");
	}

	@Test
	public void addInSyncShouldHaveHtmlBodyWhenHtmlThenMimeBodyPreferenceAndHtmlMultipartMixed() throws Exception {
		testUtils.appendToINBOX(greenMailUser, "eml/htmlAndAttachments.eml");
		...
		assertThat(body.isTruncated()).isFalse();
		assertThat(body.getBodyType()).isEqualTo(MSEmailBodyType.HTML);
		assertThat(CharStreams.toString(new InputStreamReader(body.getMimeData().get(), Charsets.UTF_8))).contains("html");
	}
}
